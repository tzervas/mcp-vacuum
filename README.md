...
4.  **Schema Conversion**: The MCP tool schemas (JSON Schema Draft 7) are then transformed into Kagent Custom Resource Definition (CRD) compatible format (OpenAPI v3). This includes metadata mapping, name sanitization, categorization, risk assessment, and a fail-fast option for immediate error reporting.
...
*   **Automated Network Discovery**: Employs mDNS for discovering MCP servers (SSDP planned) with async DNS resolution and overlapping discovery support.
*   **Secure OAuth 2.1 Authentication**: Implements Authorization Code Flow with PKCE (RFC 7636, using 128-character verifiers) and Dynamic Client Registration (RFC 7591).
