...
def get_windows_interfaces() -> List[str]:
    ...
    try:
        output = subprocess.check_output(
            ["netsh", "interface", "show", "interface"], universal_newlines=True
        )
        interfaces = []
...
def get_network_interfaces(skip_loopback: bool = True) -> List[str]:
    ...
    try:
        ...
        logger.warning(
            "netifaces discovery failed, trying platform-specific fallback",
            error=str(e),
        )
...
def get_interface_ips(interface: str) -> Set[str]:
    ...
    try:
        ...
        logger.error(
            "Failed to get addresses for interface", interface=interface, error=str(e)
        )
        return set()
