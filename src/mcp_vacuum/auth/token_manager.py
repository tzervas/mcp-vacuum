...
class TokenManager:
    """
    Orchestrates OAuth token acquisition, storage, and lifecycle management (caching, refresh).
    ...
    async def get_valid_oauth_token(self, server_id: str, server_info: MCPServerInfo, force_refresh: bool = False) -> Optional[OAuth2Token]:
        ...
        Args:
            ...
            server_info: MCPServerInfo object providing context like endpoints needed for auth.
                         This is crucial if client credentials or endpoints need to be discovered/fetched.
            force_refresh: If True, forces a token refresh even if a cached/stored token seems valid.

        Returns:
            A valid OAuth2Token, or None if authentication is not possible or fails.
        """
        log = self.logger.bind(server_id=server_id, server_name=server_info.name)
        log.debug("Attempting to get valid OAuth token.")

        async with await self._get_lock(server_id): # Ensure only one refresh attempt per server_id at a time
                ...
                try:
                    ...
                    if stored_token:
                        ...
                        if not stored_token.is_expired:
                            self._token_cache[server_id] = stored_token # Update cache
                            log.debug("Stored token is valid.")
                            return stored_token
                        else:
                            log.info("Stored token is expired, attempting refresh.")
                            # Proceed to refresh logic using stored_token.refresh_token
                            refreshed_token = await self._perform_token_refresh(server_id, server_info, stored_token)
                            if refreshed_token:
                                return refreshed_token
                            else:
                                # Refresh failed, token is invalid. Fall through to re-authentication or failure.
                                log.warning("Token refresh failed after finding expired stored token.")
                                await self._clear_token_data(server_id, log_context=log) # Clear invalid data
                                # Fall through to attempt full re-authentication if configured, or return None
                    else:
                        log.debug("No token found in persistent storage for this server_id.")
                except TokenNotFoundError:
                    log.debug("No token found in persistent storage (TokenNotFoundError).")
                except TokenStorageError as e:
                    log.error("Error accessing token storage", error=str(e))
                    # Depending on error, might proceed to re-auth or fail. For now, log and try re-auth.

            # 3. If force_refresh or no valid token found/refreshed, attempt full authentication flow (if applicable)
            # This part is complex as it requires either pre-configured client_id or dynamic registration,
            # and then initiating the auth code flow which requires user interaction.
            # For an automated agent, this step might mean "cannot get token automatically".
            # For now, this method focuses on stored/refreshable tokens.
            # A separate method like `initiate_new_authentication` would handle the interactive part.
            if force_refresh and not self._token_cache.get(server_id) and not await self._token_storage.get_oauth_token(server_id):
                # If force_refresh is true, but there's no token to refresh, this path is tricky.
                # It implies we need to start a new auth flow.
                log.warning("Force refresh requested, but no existing token to refresh. Full authentication flow needed (not implemented here).")
                return None # Or raise an error indicating full auth is needed.
    ...
    async def _perform_token_refresh(self, server_id: str, server_info: MCPServerInfo, expired_token: OAuth2Token) -> Optional[OAuth2Token]:
        ...
        oauth_client_cfg = await self._get_oauth_client_config(server_id, server_info, log_context=log)
        if not oauth_client_cfg:
            log.error("Failed to obtain OAuth client configuration for token refresh.")
            return None

        async with OAuth2Client(client_config=oauth_client_cfg, app_config=self.app_config) as client:
            try:
                log.info("Attempting token refresh via OAuth client.")
                new_token = await client.refresh_token(expired_token.refresh_token)
                ...
                log.error("MCPAuthError during token refresh", error=str(e), requires_reauth=getattr(e, 'requires_reauth', False))
                if getattr(e, 'requires_reauth', False): # Custom attribute in MCPAuthError if refresh token is definitively invalid
                    log.warning("Refresh token is invalid, clearing stored token data.")
                    await self._clear_token_data(server_id, log_context=log)
            except MCPConnectionError as e:
                log.error("MCPConnectionError during token refresh", error=str(e))
    ...
    async def _get_oauth_client_config(self, server_id: str, server_info: MCPServerInfo, log_context: Any) -> Optional[OAuth2ClientConfig]:
        ...
        try:
            ...
            log_context.warning("Error retrieving stored client credentials", error=str(e))

        # 2. If no stored creds, and dynamic registration is enabled, try to register
            ...
            if server_info.registration_endpoint: # MCPServerInfo needs this field
                ...
                try:
                    ...
                    async with registrar:
                         # Pass software_id and software_version from app_config if available
                        client_creds = await registrar.register_client(
                            registration_endpoint=str(server_info.registration_endpoint), # Ensure str
                    ...
                    log_context.error("Dynamic client registration failed", error=str(e))
                except MCPConnectionError as e:
                    log_context.error("Connection error during dynamic client registration", error=str(e))
            else:
                log_context.debug("Dynamic registration enabled, but server has no registration_endpoint.")
        ...
        if not final_client_id:
            log_context.error("No client_id available (neither stored, dynamically registered, nor default in config). Cannot proceed with OAuth.")
            return None

        # Determine endpoints: Prefer server_info (discovered), fallback to default config.
        # MCPServerInfo should have auth_metadata with these endpoints.
        auth_meta = server_info.auth_metadata
        auth_endpoint = auth_meta.authorization_endpoint if auth_meta and auth_meta.authorization_endpoint else (self.auth_config.oauth_default_client.authorization_endpoint if self.auth_config.oauth_default_client else None)
        token_endpoint = auth_meta.token_endpoint if auth_meta and auth_meta.token_endpoint else (self.auth_config.oauth_default_client.token_endpoint if self.auth_config.oauth_default_client else None)

        if not auth_endpoint or not token_endpoint:
            log_context.error("Authorization or Token endpoint not found for server or in default config.")
            return None

        # Redirect URI - this is tricky for an agent. Use default from config.
        redirect_uri = self.auth_config.oauth_default_client.redirect_uri if self.auth_config.oauth_default_client else f"http://localhost:{self.auth_config.oauth_redirect_uri_port}/oauth/callback"

        scopes = self.auth_config.oauth_default_client.scopes if self.auth_config.oauth_default_client else ["openid", "profile", "mcp:tools"]
    ...
    async def _clear_token_data(self, server_id: str, log_context: Any) -> None:
        ...
        try:
            ...
            log_context.error("Failed to delete token from storage during cleanup", error=str(e))
    ...
    async def clear_all_server_credentials(self, server_id: str) -> None:
        ...
        log.info("Clearing ALL credentials (OAuth token and Client Credentials) for server.")
        self._token_cache.pop(server_id, None)
        try:
            await self._token_storage.delete_oauth_token(server_id)
        ...
        try:
            ...
            log.error("Failed to delete Client Credentials from storage", error=str(e))


# Note: The interactive part of the OAuth flow (redirecting user to auth server, handling callback)
# is not handled by TokenManager directly. TokenManager expects to be given a token (e.g. via store_new_token)
# or to be able to refresh an existing one.
# The `get_valid_oauth_token` method is for non-interactive scenarios where a token might already exist or be refreshable.
# A higher-level "AuthenticationAgent" would use TokenManager and OAuth2Client to run the full flow.
# MCPServerInfo model needs to be updated to include `registration_endpoint: Optional[HttpUrl]`
# and ensure `auth_metadata` is populated with `authorization_endpoint` and `token_endpoint` if known.
# This will likely come from discovery (e.g. .well-known/oauth-authorization-server).
