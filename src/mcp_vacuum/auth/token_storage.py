...
class BaseTokenStorage(abc.ABC):
    """Abstract base class for storing and retrieving OAuth tokens and client
    credentials."""
    ...
    async def store_client_credentials(
        self, server_id: str, credentials: ClientCredentials
    ) -> None:
        """Stores ClientCredentials for a given server ID (e.g., after dynamic
        registration)."""
        pass
...
class KeyringTokenStorage(BaseTokenStorage):
    ...
    Uses separate entries for OAuth tokens and client credentials, prefixed by
    server_id.
    """
    ...
    def __init__(self, auth_config: AuthConfig):
        ...
        self.logger = logger.bind(
            storage_type="keyring", service_name=self.service_name
        )
    ...
    async def _store_item(
        self,
        key: str,
        item_model: type[OAuth2Token] | type[ClientCredentials],
        ...
        try:
            ...
            # Avoid logging full key if sensitive
            self.logger.debug("Stored item in keyring", key_hint=key[:10] + "...")
        except keyring.errors.NoKeyringError:
            self.logger.error(
                "No keyring backend found. Please install a keyring provider "
                "(e.g., SecretService, Windows Credential Manager)."
            )
            raise TokenStorageError("No keyring backend available.") from None
        except Exception as e:
            self.logger.error(
                "Failed to store item in keyring",
                key_hint=key[:10] + "...",
                error=str(e),
            )
    ...
    async def _get_item(
        ...
        try:
            ...
            self.logger.error(
                "No keyring backend found when trying to retrieve item."
            )
            raise TokenStorageError("No keyring backend available.") from None
        except json.JSONDecodeError as e:
            self.logger.warning(
                "Failed to decode JSON from keyring for item",
                key_hint=key[:10] + "...",
                error=str(e),
            )
            ...
            self.logger.warning(
                "Failed to validate item data from keyring",
                key_hint=key[:10] + "...",
                error=str(e),
            )
            ...
            self.logger.error(
                "Failed to retrieve item from keyring",
                key_hint=key[:10] + "...",
                error=str(e),
            )
            raise TokenStorageError(
                f"Failed to retrieve item from keyring for key '{key}': {e}"
            ) from e
    ...
    async def _delete_item(self, key: str) -> None:
        try:
            ...
            # This error might mean the password/item didn't exist. Not always
            # a critical failure.
            self.logger.warning(
                "Item not found or could not be deleted from keyring",
                key_hint=key[:10] + "...",
            )
            pass  # Fail silently or raise specific error if needed
        except keyring.errors.NoKeyringError:
            self.logger.error(
                "No keyring backend found when trying to delete item."
            )
            raise TokenStorageError("No keyring backend available.") from None
        except Exception as e:
            self.logger.error(
                "Failed to delete item from keyring",
                key_hint=key[:10] + "...",
                error=str(e),
            )
            raise TokenStorageError(
                f"Failed to delete item from keyring for key '{key}': {e}"
            ) from e
...
class EncryptedFileTokenStorage(BaseTokenStorage):
    ...
    Uses Fernet encryption. Requires an encryption key (derive from password or
    env var).
    WARNING: Managing the encryption key securely is critical.
             ...
             or from a securely managed environment variable (e.g., via Docker
             secrets, k8s secrets).
    """
    ...
    #   - One option: Use a password provided by user/env, derive key using
    #     PBKDF2HMAC.
    #   - Store salt alongside encrypted data if PBKDF2 is used.
    # - File structure: JSON dictionary where keys are server_ids, values are
    #   dicts of tokens/creds.
    #   {
    #     "server1_oauth_token": "encrypted_oauth_token_json_for_server1",
    #     "server1_client_creds": "encrypted_client_creds_json_for_server1",
    #     ...
    #   }
    # - Need to read the whole file, decrypt, modify, encrypt, write back.
    #   (Consider atomicity).
    ...
    def __init__(self, auth_config: AuthConfig, encryption_key: bytes):
        ...
        # Encryption key must be urlsafe-base64-encoded 32-byte key
        self.fernet = Fernet(encryption_key)
        self.logger = logger.bind(
            storage_type="encrypted_file", file_path=str(self.file_path)
        )
        # Ensure file path directory exists, handle file locking for concurrent
        # access if necessary (complex for async)
    ...
    def derive_key_from_password(password: str, salt: bytes) -> bytes:
        ...
        if not salt or len(salt) < 16:  # Salt should be at least 16 bytes
            raise ValueError(
                "Salt must be provided and be at least 16 bytes for key derivation."
            )
        kdf = PBKDF2HMAC(
    ...
    # Actual implementation of store/get/delete methods for
    # EncryptedFileTokenStorage would involve file I/O, locking (e.g. aiofiles,
    # fasteners for async context), and careful handling of
    # encryption/decryption. This is non-trivial.
    # For P0, KeyringTokenStorage is likely sufficient if a keyring backend is
    # available.
    # A full implementation of EncryptedFileTokenStorage is deferred for brevity
    # here but the structure is outlined.

    async def store_oauth_token(self, server_id: str, token: OAuth2Token) -> None:
        self.logger.warning(
            "EncryptedFileTokenStorage.store_oauth_token is not fully implemented."
        )
        raise NotImplementedError(
            "EncryptedFileTokenStorage is not fully implemented."
        )

    async def get_oauth_token(self, server_id: str) -> OAuth2Token | None:
        self.logger.warning(
            "EncryptedFileTokenStorage.get_oauth_token is not fully implemented."
        )
        raise NotImplementedError(
            "EncryptedFileTokenStorage is not fully implemented."
        )
    ...
    async def delete_oauth_token(self, server_id: str) -> None:
        self.logger.warning(
            "EncryptedFileTokenStorage.delete_oauth_token is not fully implemented."
        )
        raise NotImplementedError(
            "EncryptedFileTokenStorage is not fully implemented."
        )
    ...
    async def store_client_credentials(
        ...
        self.logger.warning(
            "EncryptedFileTokenStorage.store_client_credentials is not "
            "fully implemented."
        )
        raise NotImplementedError(
            "EncryptedFileTokenStorage is not fully implemented."
    ...
    async def get_client_credentials(
        ...
        self.logger.warning(
            "EncryptedFileTokenStorage.get_client_credentials is not "
            "fully implemented."
        )
        raise NotImplementedError(
            "EncryptedFileTokenStorage is not fully implemented."
    ...
    async def delete_client_credentials(self, server_id: str) -> None:
        self.logger.warning(
            "EncryptedFileTokenStorage.delete_client_credentials is not "
            "fully implemented."
        )
        raise NotImplementedError(
            "EncryptedFileTokenStorage is not fully implemented."
...
def get_token_storage(auth_config: AuthConfig) -> BaseTokenStorage:
    ...
    if auth_config.token_storage_method == "keyring":
        ...
        # if not key_from_env:
        #     raise TokenStorageError(
        #         "Encryption key for file storage not found in env var "
        #         "MCP_VACUUM_ENCRYPTION_KEY."
        #     )
        # encryption_key = key_from_env.encode() # Assuming base64 encoded
        # For now, as it's not fully implemented:
        logger.error(
            "EncryptedFileTokenStorage is selected but not fully implemented "
            "or encryption key management is pending."
        )
        raise NotImplementedError(
            "EncryptedFileTokenStorage key management needs secure implementation."
        )
        # return EncryptedFileTokenStorage(auth_config, encryption_key)
    else:
        logger.error(
            f"Unsupported token_storage_method: {auth_config.token_storage_method}"
        )
        raise ValueError(
            f"Unsupported token_storage_method: {auth_config.token_storage_method}"
        )
